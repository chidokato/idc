...
<div class="d-sm-flex align-items-center justify-content-between mb-3 flex">
    ...
    <a class="add-iteam" href="{{route('internalcategory.create')}}"><button class="btn-success form-control" type="button"><i class="fa fa-plus" aria-hidden="true"></i> {{__('lang.add')}}</button></a>
</div>
            ...
            { 
                ?>
                    <tr id="category" style="border-bottom: 1px solid #f3f6f9;">
                        ...
                        <td><a href="{{route('internalcategory.edit',[$val->id])}}">{{$str}}{{$val->name}}</a></td>
                        <td>{{$val->slug}}</td>
                        ...
                        <td style="display: flex;">
                            ...
                            <a href="{{route('internalcategory.edit',[$val->id])}}" class="mr-3"><i class="fas fa-edit" aria-hidden="true"></i></a>
                            <form action="{{route('internalcategory.destroy', [$val->id])}}" method="POST">
                              @method('DELETE')
                              @csrf
